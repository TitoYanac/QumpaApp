...
import 'historial_en_espera_detalle.dart';

class HistorialEnEspera extends StatefulWidget {
  final CuentaUsuario usuarioEmpresa;
  const HistorialEnEspera(this.usuarioEmpresa, {Key? key}) : super(key: key);

  @override
  _HistorialEnEsperaState createState() => _HistorialEnEsperaState();
}

class _HistorialEnEsperaState extends State<HistorialEnEspera> {
  late double ancho;
  late double aux;
  late double alto;
...
/*
          ...
          print("data[0]['FALSE']: ${data[0]['FALSE']}");
          print("data[0]: ${data[0]['idTipoEnvio']}");
          print("tiposDeEnvio 0: ${tiposDeEnvio[0]}");
          print("tiposDeEnvio 1: ${tiposDeEnvio[1]}");
          */
//print("tiposDeEnvio 2: ${tiposDeEnvio[2]}");
                    ...
                    child: ListTile(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => HistorialEnEsperaDetalle(
                                  widget.usuarioEmpresa, misFichas[index])),
                        );
                      },
                    ),
                  ),
  ...
  Future cargarEnviosEnReparto(CuentaUsuario miUsuario, ancho, context) async {
    ...
    print("cargarEnviosEnReparto");
    print("id_Empresa: ${miUsuario.idCuentaUsuario}");


    String url = "$servidor/Empresa/testing.php";

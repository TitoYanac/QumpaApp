...
import 'historial_en_reparto_detalle.dart';

class HistorialEnReparto extends StatefulWidget {
  final CuentaUsuario usuarioConductor;
  const HistorialEnReparto(this.usuarioConductor, {Key? key}) : super(key: key);

  @override
  _HistorialEnRepartoState createState() => _HistorialEnRepartoState();
}

class _HistorialEnRepartoState extends State<HistorialEnReparto> {
  late double ancho;
  late double aux;
  late double alto;
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: cargarEnviosEnReparto(widget.usuarioConductor, ancho, context),
                    ...
                    child: ListTile(
                      ...
                      onTap: () {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => HistorialConductorEnRepartoDetalle(
                                  widget.usuarioConductor, misDeliverys[index])),
                        );
  ...
  Future cargarEnviosEnReparto(CuentaUsuario miUsuario, ancho, context) async {
    String url = "$servidor/Conductor/App_mostrar_envios_en_reparto.php";

    final response = await http.post(Uri.parse(url), body: {
      "id_Conductor": miUsuario.vehiculo.chofer.idChofer,
    });
